...
// peer.Close().
func (peer *Peer) LoopRounds(roundType string, roundType2 string, rounds int) {
	...
	for {
		...
		case <-ticker.C:
			if peer.LastRound() >= rounds && rounds >= 0 {
				...
				if peer.IsRoot(peer.ViewNo) {
					dbg.Lvl3(peer.Name(), "BitCoSi root server in round", peer.LastRound()+1, "of", rounds)
					round, err := sign.NewRoundFromType(roundType, peer.Node)
					if err != nil {
						dbg.Fatal("Couldn't create", roundType, err)
					}
					round2, err2 := sign.NewRoundFromType(roundType2, peer.Node)
					if err2 != nil {
						dbg.Fatal("Couldn't create", roundType2, err)
					}
...
func (peer *Peer) ConcurrentAnnouncement(round sign.Round, wg *sync.WaitGroup) {
